...
namespace Tir1
{
    public partial class LeaderbordForm : KryptonForm
    {
        osulvl olvl;
        account ac;
        List<String> acs;
        ...
        public LeaderbordForm(osulvl olvl)
        {
            ...
            for (int i = 0; i < acs.Count; i++)
            {
                ...
                if (SortBox.SelectedIndex == 0)
                {
                    ...
                    for (int j = 0; j < tempac.lvls.Count; j++)
                        if (olvl.name == tempac.lvls[j].name && tempac.lvls[j].complete)
                        {
                            s = tempac.lvls[j].recordsct;
                            str = tempac.lvls[j].recordmark;
                            break;
                        }
                    arr.Add(s);
                    marks.Add(str);
                ...
                if (SortBox.SelectedIndex == 1)
                {
                    ...
                    string str = "";
                    for (int j = 0; j < tempac.lvls.Count; j++)
                        if (olvl.name == tempac.lvls[j].name)
                        {
                            s = tempac.lvls[j].practicerecord;
                            str = tempac.lvls[j].practicemark;
                            break;
                        }
                    arr.Add(s);
                    marks.Add(str);
        ...
        public void SetButtons()
        {
            ...
            for (int i = 0; i < acs.Count; i++)
            {
                ...
                labels[i].TabIndex = 4;
                labels[i].Text = (i + 1).ToString() + ". " + acs[i] + " — " + arr[i] + " " + marks[i];
        ...
        private void LeaderbordForm_FormClosed(object sender, FormClosedEventArgs e)
        {
        ...
        private void LeaderbordForm_Load(object sender, EventArgs e)
        {
